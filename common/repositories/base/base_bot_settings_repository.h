...
public:
	...
	)
	{
		for (auto &bot_settings : bot_settingss) {
			if (bot_settings.bot_id == bot_settings_id) {
				return bot_settings;
			}
		}
	...
	)
	{
		...
		if (results.RowCount() == 1) {
			...
			e.setting_id	= row[1] ? static_cast<uint32_t>(strtoul(row[1], nullptr, 10)) : 0;
			e.setting_type	= row[2] ? static_cast<uint32_t>(strtoul(row[2], nullptr, 10)) : 0;
			e.value			= row[3] ? static_cast<uint32_t>(strtoul(row[3], nullptr, 10)) : 0;
	...
	)
	{
		...
		v.push_back(columns[1] + " = " + std::to_string(e.setting_id));
		v.push_back(columns[2] + " = " + std::to_string(e.setting_type));
		v.push_back(columns[3] + " = " + std::to_string(e.value));
	...
	static std::vector<BotSettings> All(Database& db)
	{
		...
		for (auto row = results.begin(); row != results.end(); ++row) {
			...
			e.bot_id		= row[0] ? static_cast<uint32_t>(strtoul(row[0], nullptr, 10)) : 0;
			e.setting_id	= row[1] ? static_cast<uint8_t>(strtoul(row[1], nullptr, 10)) : 0;
			e.setting_type	= row[2] ? static_cast<uint32_t>(strtoul(row[2], nullptr, 10)) : 0;
			e.value			= row[3] ? static_cast<uint32_t>(strtoul(row[3], nullptr, 10)) : 0;
