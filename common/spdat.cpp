...
{
	if (RuleI(Spells, ResurrectionEffectsBlock) == RES_EFFECTS_BLOCK) {
		LogSpells("ResurrectionEffectsBlock triggered -- [{}] is blocked by [{}]", spells[spell_id2].name, spells[spell_id1].name);
		return -1;	// can't stack
	}
	else if (RuleI(Spells, ResurrectionEffectsBlock) == RES_EFFECTS_BLOCK_WITH_BUFFS) {
		LogSpells("ResurrectionEffectsBlock triggered -- [{}] is blocked by [{}], moving to empty slot if available", spells[spell_id2].name, spells[spell_id1].name);
		return 2;	// move to empty slot if available
	}
