...
void bot_command_charm(Client* c, const Seperator* sep)
{
	...
	if (my_bot->CheckSpellRecastTimers(botSpell.SpellId)) {
...
void bot_command_click_item(Client *c, const Seperator *sep)
{
	...
	if (helper_is_help_or_usage(sep->arg[1])) {
		...
		c->Message(Chat::White, "Note: Items without unlimited charges are not currently supported.");
		c->Message(Chat::White, "Note: Items with unlinked recast timers (-1) are not currently supported.");
		return;
	}
...
void DefaultBotSettings(Bot* my_bot, uint8 chosen_type)
{
	...
	if (chosen_type == DEFAULT_MISC || chosen_type == DEFAULT_ALL) {
		...
		my_bot->SetHoldDS(false);
		my_bot->SetHoldResists(false);
	...
	if (chosen_type == DEFAULT_HOLDS || chosen_type == DEFAULT_ALL) {
		...
		my_bot->SetHoldCures(false);
		my_bot->SetHoldDebuffs(false);
		my_bot->SetHoldDispels(true);
		my_bot->SetHoldDoTs(false);
		my_bot->SetHoldEscapes(false);
		my_bot->SetHoldFastHeals(false);
		my_bot->SetHoldGroupHeals(false);
		my_bot->SetHoldHateRedux(false);
		my_bot->SetHoldHeals(false);
		my_bot->SetHoldHotHeals(false);
		my_bot->SetHoldInCombatBuffs(false);
		my_bot->SetHoldInCombatBuffSongs(false);
		my_bot->SetHoldLifetaps(false);
		my_bot->SetHoldLulls(false);
		my_bot->SetHoldMez(false);
		my_bot->SetHoldNukes(false);
		my_bot->SetHoldOutOfCombatBuffSongs(false);
		my_bot->SetHoldPets(false);
		my_bot->SetHoldPetBuffs(false);
		my_bot->SetHoldPetHeals(false);
		my_bot->SetHoldPreCombatBuffs(false);
		my_bot->SetHoldPreCombatBuffSongs(false);
		my_bot->SetHoldRegularHeals(false);
		my_bot->SetHoldRez(false);
		my_bot->SetHoldRoots(true);
		my_bot->SetHoldSlows(false);
		my_bot->SetHoldSnares(false);
	}
