...
bool Mob::HasIllusionsBlocked() {
	if (IsClient()) {
		return CastToClient()->GetIllusionBlock();
	}
	if (IsBot()) {
		return CastToBot()->GetIllusionBlock();
	}
