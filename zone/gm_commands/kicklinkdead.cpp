...
void command_kick_linkdead(Client* c, const Seperator* sep)
{
	...
	if (!arguments) {
		c->Message(Chat::White, "Usage: #kicklinkdead [Character Name]");
		return;
	}
	...
	if (client) {
		if (client->Admin() <= c->Admin()) {
			...
			safe_delete(outapp);
			client->KickLinkDead("Ordered kick linkdead by command");
			c->Message(
				Chat::White,
				fmt::format(
					"{} has been kicked from the server and sent into a LinkDead state.",
					character_name
				).c_str()
			);
		}
