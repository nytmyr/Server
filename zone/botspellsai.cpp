...
bool Bot::AICastSpell(Mob* tar, uint8 iChance, uint16 spellType) {
	...
	if (
		...
		(IS_PET_SPELL_TYPE(spellType) && !tar->IsPet()) ||
		...
		(tar->IsPet() && tar->IsCharmed() && IS_HEAL_SPELL_TYPE(spellType) && !RuleB(Bots, AllowCharmedPetHeals))
	) {
		return false;
	}
	...
	switch (spellType) {
		...
		case BotSpellTypes::ResistBuffs:
			...
			if (GetClass() != Class::Shaman && spellType == BotSpellTypes::InCombatBuff && IsCasterClass(GetClass()) && GetLevel() >= GetStopMeleeLevel()) {
				return false;
			}
		...
		case BotSpellTypes::OutOfCombatBuffSong:
			if (IsEngaged() || tar->IsEngaged()) { // Out-of-Combat songs can not be cast in combat
				return false;
			}
	...
	if (botCaster && botCaster->AI_HasSpells()) {
		...
		for (int i = botSpellList.size() - 1; i >= 0; i--) {
			...
			if (
				...
				if (AE) {
					if (!botCaster->HasValidAETarget(botCaster, botSpellList[i].spellid, spellType, tar)) {
						continue;
					}
...
Mob* Bot::GetFirstIncomingMobToMez(Bot* botCaster, int16 spellid, uint16 spellType, bool AE) {
	...
	if (botCaster) {
		...
		for (auto& close_mob : botCaster->close_mobs) {
			...
			if (npc->IsMezzed() || npc->GetBotAttackFlag()) {
				continue;
			}
			...
			if (!botCaster->CheckLosCheat(botCaster, npc)) {
				continue;
			}
			...
			for (Mob* m : v) {
				if (npc->IsOnHatelist(m)) {
					if (m->GetTarget() == npc) {
						if (
							m->IsBot() ||
							(m->IsCasting() && (IsEffectInSpell(m->CastingSpellID(), SE_CurrentHP) || IsEffectInSpell(m->CastingSpellID(), SE_CurrentHPOnce))) ||
	...
	if (botCaster) {
		...
		for(std::list<BotSpell>::iterator botSpellListItr = botSpellList.begin(); botSpellListItr != botSpellList.end(); ++botSpellListItr) {
			...
			if (IsPureNukeSpell(botSpellListItr->SpellId) || IsDamageSpell(botSpellListItr->SpellId)) {
				...
				if (!botCaster->CastChecks(botSpellListItr->SpellId, tar, spellType)) {
					continue;
				}

				if (AE) {
					if (!botCaster->HasValidAETarget(botCaster, botSpellListItr->SpellId, spellType, tar)) {
						continue;
					}
...
void Bot::CheckBotSpells() {
	...
	for (const auto& s : spellList) {
		if (!IsValidSpell(s.spellid)) {
			LogBotSpellTypeChecks("{} is an invalid spell", s.spellid); //deleteme
			continue;
		}
