...
bool Bot::CheckLoreConflict(const EQ::ItemData* item) {
	...
	if (RuleI(Inventory, CustomItemsLoreIDStart) == 0 && RuleI(Inventory, SecondCustomItemsLoreIDStart) == 0) {
		...
		if (item->LoreGroup == -1) { // Standard lore items; look everywhere except the shared bank, return the result
			...
			uint32 secondcustomitems = RuleI(Inventory, SecondCustomItemsLoreIDStart);
			//do checks for customitems against regular items
			if (item->ID >= customitems && item->ID < secondcustomitems) {
				if (m_inv.HasItem(dupecheck - customitems, 0, ~invWhereSharedBank) != INVALID_INDEX)
					return (m_inv.HasItem(dupecheck - customitems, 0, ~invWhereSharedBank) != INVALID_INDEX);
				if (m_inv.HasItem(dupecheck + (secondcustomitems - customitems), 0, ~invWhereSharedBank) != INVALID_INDEX)
					return (m_inv.HasItem(dupecheck + (secondcustomitems - customitems), 0, ~invWhereSharedBank) != INVALID_INDEX);
			}
			if (item->ID < customitems) {
				if (m_inv.HasItem(dupecheck + customitems, 0, ~invWhereSharedBank) != INVALID_INDEX)
					return (m_inv.HasItem(dupecheck + customitems, 0, ~invWhereSharedBank) != INVALID_INDEX);
				if (m_inv.HasItem(dupecheck + secondcustomitems, 0, ~invWhereSharedBank) != INVALID_INDEX)
					return (m_inv.HasItem(dupecheck + secondcustomitems, 0, ~invWhereSharedBank) != INVALID_INDEX);
			}
			//do checks for secondcustomitems against regular items
			if (item->ID >= secondcustomitems) {
				if (m_inv.HasItem(dupecheck - secondcustomitems, 0, ~invWhereSharedBank) != INVALID_INDEX)
					return (m_inv.HasItem(dupecheck - secondcustomitems, 0, ~invWhereSharedBank) != INVALID_INDEX);
				if (m_inv.HasItem(dupecheck - (secondcustomitems - customitems), 0, ~invWhereSharedBank) != INVALID_INDEX)
					return (m_inv.HasItem(dupecheck - (secondcustomitems - customitems), 0, ~invWhereSharedBank) != INVALID_INDEX);
...
bool EntityList::Bot_AICheckCloseBeneficialSpells(Bot* caster, uint8 iChance, float iRange, uint32 iSpellTypes) {
	...
	if (iSpellTypes == SpellType_Buff) {
		uint8 chanceToCast = caster->IsEngaged() ? 0 : 100;
...
bool Bot::UseDiscipline(uint32 spell_id, uint32 target) {
	...
	if(spell.recast_time > 0) {
		if(CheckDisciplineReuseTimers(spell_id)) {
			if (spells[spell_id].timer_id > 0) {
				SetDisciplineRecastTimer(spell_id);
			}
		} else {
...
void Bot::StopMoving()
{
	...
	SetCombatOutOfRangeJitterFlag(false);
	Mob::StopMoving();
}
...
void Bot::SetSpellRecastTimer(uint16 spell_id, int32 recast_delay) {
	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to set spell recast timer.'", GetCleanName());
		LogSpells("Spell [{}] failed to set spell recast timer.", spell_id);
		return;
	}
	...
	t.is_item = false;
	t.item_id = 0;
	TestDebug("SETTING SPELL for {} - [ID: {}] [Time: {}] [Expires: {}] [Type: {}] [ID: {}]", GetCleanName(), t.timer_id, Timer::GetCurrentTime(), t.timer_value, (t.is_spell ? "Spell" : t.is_disc ? "Disc" : t.is_item ? "Item" : "Invalid"), (t.is_item ? t.item_id : t.spell_id)); //deleteme
	bot_timers.push_back(t);
}

int32 Bot::GetSpellRecastTimer(uint16 spell_id) {
	int32 result = 0;

	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to get spell recast timer.'", GetCleanName());
		LogSpells("Spell [{}] failed to get spell recast timer.", spell_id);
		return result;
	}

	if (!bot_timers.empty()) {
		for (int i = 0; i < bot_timers.size(); i++) {
			if (bot_timers[i].is_spell && bot_timers[i].timer_id == spells[spell_id].timer_id) {
...
bool Bot::CheckSpellRecastTimers(uint16 spell_id) {
	ClearExpiredTimers();

	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to check spell recast timer.'", GetCleanName());
		LogSpells("Spell [{}] failed to check spell recast timer.", spell_id);
		return false;
	}
...
void Bot::SetDisciplineRecastTimer(uint16 spell_id, int32 recast_delay) {
	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to set discipline reuse timer.'", GetCleanName());
		LogSpells("Spell [{}] failed to set discipline reuse timer.", spell_id);
		return;
	}
	...
	t.is_item = false;
	t.item_id = 0;
	TestDebug("SETTING DISC for {} - [ID: {}] [Time: {}] [Expires: {}] [Type: {}] [ID: {}]", GetCleanName(), t.timer_id, Timer::GetCurrentTime(), t.timer_value, (t.is_spell ? "Spell" : t.is_disc ? "Disc" : t.is_item ? "Item" : "Invalid"), (t.is_item ? t.item_id : t.spell_id)); //deleteme
	bot_timers.push_back(t);
}

int32 Bot::GetDisciplineReuseTimer(uint16 spell_id) {
	int32 result = 0;

	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to get discipline reuse timer.'", GetCleanName());
		LogSpells("Spell [{}] failed to get discipline reuse timer.", spell_id);
		return result;
	}

	if (!bot_timers.empty()) {
		for (int i = 0; i < bot_timers.size(); i++) {
			if (bot_timers[i].is_disc && bot_timers[i].timer_id == spells[spell_id].timer_id) {
				result = bot_timers[i].timer_value;
				break;
			}
		}
...
uint32 Bot::GetDisciplineReuseRemainingTime(uint16 spell_id) {
	...
	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to get discipline reuse remaining time.'", GetCleanName());
		LogSpells("Spell [{}] failed to get discipline reuse remaining time.", spell_id);
		return result;
	}
...
bool Bot::CheckDisciplineReuseTimers(uint16 spell_id) {
	ClearExpiredTimers();

	if (!IsValidSpell(spell_id)) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to check discipline reuse timer.'", GetCleanName());
		LogSpells("Spell [{}] failed to check discipline reuse timer.", spell_id);
		return false;
	}
...
void Bot::SetItemReuseTimer(const EQ::ItemData* item, uint32 reuse_timer) {

	if (!item) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to set item reuse timer.'", GetCleanName());
		LogError("Failed to set item reuse timer.");
		return;
	}
	...
	t.timer_value = (reuse_timer != 0 ? reuse_timer : (Timer::GetCurrentTime() + (item->RecastDelay * 1000)));
	...
	t.is_item = true;
	t.item_id = item->ID;
	TestDebug("SETTING ITEM for {} - [ID: {}] [Time: {}] [Expires: {}] [Type: {}] [ID: {}]", GetCleanName(), t.timer_id, Timer::GetCurrentTime(), t.timer_value, (t.is_spell ? "Spell" : t.is_disc ? "Disc" : t.is_item ? "Item" : "Invalid"), (t.is_item ? t.item_id : t.spell_id)); //deleteme
	bot_timers.push_back(t);
}

int32 Bot::GetItemReuseTimer(const EQ::ItemData* item) {
	int32 result = 0;

	if (!item) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to get item reuse timer.'", GetCleanName());
		LogError("Failed to get item reuse timer.");
		return result;
	}
	...
	if (!bot_timers.empty()) {
		for (int i = 0; i < bot_timers.size(); i++) {
			if (bot_timers[i].is_item) {
				if (item->RecastType == NegativeItemReuse && bot_timers[i].timer_id == item->ID) {
					result = bot_timers[i].timer_value;
					break;
				}
				else if (item->RecastType != NegativeItemReuse && bot_timers[i].timer_id == item->RecastType) {
					result = bot_timers[i].timer_value;
					break;
				}
			}
...
bool Bot::CheckItemReuseTimers(const EQ::ItemData* item) {
	ClearExpiredTimers();

	if (!item) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to check reuse timer.'", GetCleanName());
		LogError("Failed to check item reuse timer.");
		return false;
	}

	if (GetItemReuseTimer(item) < Timer::GetCurrentTime()) {
		return true;
	}
...
uint32 Bot::GetItemReuseRemainingTime(const EQ::ItemData* item) {
	int32 result = 0;

	if (!item) {
		GetOwner()->Message(Chat::Red, "%s says, 'Failed to get item reuse timer remaining time.'", GetCleanName());
		LogError("Failed to get item reuse timer remaining time.");
		return result;
	}

	if (GetItemReuseTimer(item) > Timer::GetCurrentTime()) {
		result = (GetItemReuseTimer(item) - Timer::GetCurrentTime());
	}
...
void Bot::TryItemClick(uint16 slot_id) {
	...
	const EQ::ItemInstance* inst = GetClickItem(slot_id);
	...
	if (!CheckItemReuseTimers(item)) {
		uint32 remaining_time = (GetItemReuseRemainingTime(item) / 1000);
		GetOwner()->Message(Chat::Yellow,
			fmt::format(
				"{} says, 'I can use this item in {}'.",
				GetCleanName(),
				Strings::SecondsToTime(remaining_time)
			).c_str()
		);
...
EQ::ItemInstance* Bot::GetClickItem(uint16 slot_id) {
	...
	if (item->ID == MAG_EPIC_1_0 && !RuleB(Bots, CanClickMageEpicV1)) {
		GetOwner()->Message(Chat::Red, "%s says, '%s is wrong version. I can only use the bot version of this item. Speak to The Keeper of Secrets for more information.'", GetCleanName(), item->Name);
		return nullptr;
	}

	if (item->Click.Effect <= 0) {
		GetOwner()->Message(Chat::Red, "%s says, '%s does not have a clickable effect.'", GetCleanName(), item->Name);
		return nullptr;
	}

	if (!IsValidSpell(item->Click.Effect)) {
		GetOwner()->Message(Chat::Red, "%s says, '%s does not have a valid clickable effect.'", GetCleanName(), item->Name);
		return nullptr;
	}

	if (item->ReqLevel > GetLevel()) {
		GetOwner()->Message(Chat::Red, "%s says, 'I am below the level requirement of %i for %s.'", GetCleanName(), item->ReqLevel, item->Name);
		return nullptr;
	}

	if (item->Click.Level2 > GetLevel()) {
		GetOwner()->Message(Chat::Red, "%s says, 'I must be level %i to use %s.'", GetCleanName(), item->Click.Level2, item->Name);
		return nullptr;
	}

	if (inst->GetCharges() == 0) {
		GetOwner()->Message(Chat::Red, "%s says, '%s is out of charges.'", GetCleanName(), item->Name);
		return nullptr;
	}
...
void Bot::DoItemClick(const EQ::ItemData* item, uint16 slot_id) {
	...
	if (tar != this && !GetOwner()->DoLosChecks(GetOwner(), tar)) {
		GetOwner()->Message(Chat::Red, "You must have Line of Sight to use this command.");
		return;
	}
	...
	if (GetClass() == BARD) {
		...
		if (!CastSpell(item->Click.Effect, tar->GetID(), EQ::spells::CastingSlot::Item, item->CastTime, 0, 0, slot_id)) {
			GetOwner()->Message(Chat::Red, "%s says, 'Casting failed for %s. This could be due to zone restrictions, target restrictions or other limiting factors.", GetCleanName(), item->Name);
		}
	}
...
void Bot::ClearExpiredTimers() {
	std::vector<BotTimer_Struct> timer_entry = bot_timers;

	if (!bot_timers.empty()) {
		int current = 0;
		int end = bot_timers.size();
		while (current < end) {
			TestDebugDetail("CHECKING {} timer [{}/{}] - [ID: {}] [Time: {}] [Expires: {}] [Type: {}] [ID: {}]", GetCleanName(), current + 1, end, bot_timers[current].timer_id, Timer::GetCurrentTime(), bot_timers[current].timer_value, (bot_timers[current].is_spell ? "Spell" : bot_timers[current].is_disc ? "Disc" : bot_timers[current].is_item ? "Item" : "Invalid"), (bot_timers[current].is_item ? bot_timers[current].item_id : bot_timers[current].spell_id)); //deleteme
			if (bot_timers[current].timer_value < Timer::GetCurrentTime()) {
				TestDebugDetail("EXPIRED - [ID: {}] [Time: {}] [Expires: {}] [Type: {}] [ID: {}]", bot_timers[current].timer_id, Timer::GetCurrentTime(), bot_timers[current].timer_value, (bot_timers[current].is_spell ? "Spell" : bot_timers[current].is_disc ? "Disc" : bot_timers[current].is_item ? "Item" : "Invalid"), (bot_timers[current].is_item ? bot_timers[current].item_id : bot_timers[current].spell_id)); //deleteme
