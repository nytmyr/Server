...
bool BotDatabase::LoadBot(const uint32 bot_id, Bot*& loaded_bot)
{
	...
	if (loaded_bot) {
		...
		loaded_bot->SetHoldHateRedux(l.hold_hateredux);
		loaded_bot->SetHoldHeals(l.hold_heals);
		loaded_bot->SetHoldHotHeals(l.hold_hot_heals);
		...
		loaded_bot->SetCompleteHealDelay(l.complete_heal_delay);
		loaded_bot->SetCureDelay(l.cure_delay);
		loaded_bot->SetDebuffDelay(l.debuff_delay);
		loaded_bot->SetDispelDelay(l.dispel_delay);
		loaded_bot->SetDotDelay(l.dot_delay);
		loaded_bot->SetEscapeDelay(l.escape_delay);
		loaded_bot->SetFastHealDelay(l.fast_heal_delay);
		loaded_bot->SetHateReduxDelay(l.hate_redux_delay);
		loaded_bot->SetHealDelay(l.heal_delay);
		loaded_bot->SetHotHealDelay(l.hot_heal_delay);
		loaded_bot->SetInCombatBuffDelay(l.incombatbuff_delay);
		loaded_bot->SetLifetapDelay(l.lifetap_delay);
		loaded_bot->SetMezDelay(l.mez_delay);
		loaded_bot->SetNukeDelay(l.nuke_delay);
		loaded_bot->SetRootDelay(l.root_delay);
		loaded_bot->SetSlowDelay(l.slow_delay);
		loaded_bot->SetSnareDelay(l.snare_delay);
		loaded_bot->SetBuffThreshold(l.buff_threshold);
		loaded_bot->SetCompleteHealThreshold(l.complete_heal_threshold);
		loaded_bot->SetCureThreshold(l.cure_threshold);
		loaded_bot->SetDebuffThreshold(l.debuff_threshold);
		loaded_bot->SetDispelThreshold(l.dispel_threshold);
		loaded_bot->SetDotThreshold(l.dot_threshold);
		loaded_bot->SetEscapeThreshold(l.escape_threshold);
		loaded_bot->SetFastHealThreshold(l.fast_heal_threshold);
		loaded_bot->SetHateReduxThreshold(l.hate_redux_threshold);
		loaded_bot->SetHealThreshold(l.heal_threshold);
		loaded_bot->SetHotHealThreshold(l.hot_heal_threshold);
		loaded_bot->SetInCombatBuffThreshold(l.incombatbuff_threshold);
		loaded_bot->SetLifetapThreshold(l.lifetap_threshold);
		loaded_bot->SetMezThreshold(l.mez_threshold);
		loaded_bot->SetNukeThreshold(l.nuke_threshold);
		loaded_bot->SetRootThreshold(l.root_threshold);
		loaded_bot->SetSlowThreshold(l.slow_threshold);
		loaded_bot->SetSnareThreshold(l.snare_threshold);
		loaded_bot->SetBuffMinThreshold(l.buff_min_threshold);
		loaded_bot->SetCureMinThreshold(l.cure_min_threshold);
		loaded_bot->SetDebuffMinThreshold(l.debuff_min_threshold);
		loaded_bot->SetDispelMinThreshold(l.dispel_min_threshold);
		loaded_bot->SetDotMinThreshold(l.dot_min_threshold);
		loaded_bot->SetEscapeMinThreshold(l.escape_min_threshold);
		loaded_bot->SetHateReduxMinThreshold(l.hate_redux_min_threshold);
		loaded_bot->SetInCombatBuffMinThreshold(l.incombatbuff_min_threshold);
		loaded_bot->SetLifetapMinThreshold(l.lifetap_min_threshold);
		loaded_bot->SetMezMinThreshold(l.mez_min_threshold);
		loaded_bot->SetNukeMinThreshold(l.nuke_min_threshold);
...
bool BotDatabase::SaveBot(Bot* bot_inst)
{
	...
	l.hold_hateredux				= bot_inst->GetHoldHateRedux();
	l.hold_heals					= bot_inst->GetHoldHeals();
	l.hold_hot_heals				= bot_inst->GetHoldHotHeals();
	...
	l.complete_heal_delay			= bot_inst->GetCompleteHealDelay();
	...
	l.root_delay					= bot_inst->GetRootDelay();
...
bool BotDatabase::LoadTimers(Bot* bot_inst)
{
	...
	for (auto row = results.begin(); row != results.end(); ++row) {
		...
		if (timer_value < (Timer::GetCurrentTime() + recast_time)) {
			...
			//TestDebug("Set item timer for {} [ID:{}] to {} with a timer id of {}", database.GetItem(item_id)->Name, item_id, timer_value, timer_id); //deleteme
		}
	}
	...
	//TestDebug("Loaded item timers for {}", bot_inst->GetCleanName()); //deleteme
	return true;
}
...
bool BotDatabase::SaveTimers(Bot* bot_inst)
{
	...
	if (!bot_timers.empty()) {
		for (int i = 0; i < bot_timers.size(); i++) {
			...
			auto results = database.QueryDatabase(query);
			if (!results.Success()) {
				//TestDebug("Saved item timer Success failed for index {}, deleting", i); //deleteme
				DeleteTimers(bot_inst->GetBotID());
				return false;
			}
			//TestDebug("Saved item timer {} with timer_value {} and item_id {} for {} [ID:{}]", bot_timers[i].timer_id, bot_timers[i].timer_value, bot_timers[i].item_id, bot_inst->GetCleanName(), bot_inst->GetBotID()); //deleteme
		}
		//TestDebug("Saved item timers for {}", bot_inst->GetCleanName()); //deleteme
	}
...
bool BotDatabase::DeleteTimers(const uint32 bot_id)
{
	...
	//TestDebug("Deleting timers for {}", bot_id); //deleteme
...
bool BotDatabase::SaveCompleteHealDelay(const uint32 owner_id, const uint32 bot_id, const uint32 cheal_delay_value)
{
	if (!owner_id || !bot_id)
		return false;
...
bool BotDatabase::SaveFastHealThreshold(const uint32 owner_id, const uint32 bot_id, const uint8 fheal_threshold_value)
{
	if (!owner_id || !bot_id)
		return false;
	...
	query = StringFormat(
		"UPDATE `bot_data`"
		" SET `fast_heal_threshold` = '%u'"
		" WHERE `owner_id` = '%u'"
		" AND `bot_id` = '%u'",
		fheal_threshold_value,
		owner_id,
		bot_id
	);
	auto results = database.QueryDatabase(query);
...
bool BotDatabase::SaveHotHealThreshold(const uint32 owner_id, const uint32 bot_id, const uint8 hotheal_threshold_value)
{
	if (!owner_id || !bot_id)
		return false;
...
bool BotDatabase::SaveBotPetSetTypeSetting(const uint32 bot_id, const uint8 bot_petsettype_setting)
{
	if (!bot_id) {
		return false;
	}
	...
	query = fmt::format(
		"UPDATE `bot_data`"
		"SET `petsettype_setting` = {} "
		"WHERE `bot_id` = {}",
		bot_petsettype_setting,
		bot_id
	);
	auto results = database.QueryDatabase(query);
	if (!results.Success()) {
...
const char* BotDatabase::fail::SaveHoldHotHeals() { return "Failed to save hold HoT Heals status"; }
