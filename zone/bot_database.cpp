...
bool BotDatabase::SaveBotSettings(Mob* m)
{
	...
	for (uint16 i = BotBaseSettings::START; i <= BotBaseSettings::END; ++i) {
		if (m->IsBot() || (m->IsClient() && i == BotBaseSettings::IllusionBlock)) {
	...
	for (uint16 i = BotSpellTypes::START; i <= BotSpellTypes::END; ++i) {
		if (m->GetSpellHold(i) != m->GetDefaultSpellHold(i)) {
			auto e = BotSettingsRepository::BotSettings{
				.char_id = charID,
				.bot_id = botID,
				.setting_id = static_cast<uint16_t>(i),
				.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellHold),
				.value = m->GetSpellHold(i),
				.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellHold),
				.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
			};

			v.emplace_back(e);

			LogBotSettings("{} says, 'Saving {} [{}] SpellHold - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->GetDefaultSpellHold(i)); //deleteme
		}

		if (m->GetSpellDelay(i) != m->GetDefaultSpellDelay(i)) {
			auto e = BotSettingsRepository::BotSettings{
				.char_id = charID,
				.bot_id = botID,
				.setting_id = static_cast<uint16_t>(i),
				.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellDelay),
				.value = m->GetSpellDelay(i),
				.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellDelay),
				.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
			};

			v.emplace_back(e);

			LogBotSettings("{} says, 'Saving {} [{}] SpellDelay - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->GetDefaultSpellDelay(i)); //deleteme
		}

		if (m->GetSpellMinThreshold(i) != m->GetDefaultSpellMinThreshold(i)) {
			auto e = BotSettingsRepository::BotSettings{
				.char_id = charID,
				.bot_id = botID,
				.setting_id = static_cast<uint16_t>(i),
				.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellMinThreshold),
				.value = m->GetSpellMinThreshold(i),
				.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellMinThreshold),
				.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
			};

			v.emplace_back(e);

			LogBotSettings("{} says, 'Saving {} [{}] SpellMinThreshold - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->GetDefaultSpellMinThreshold(i)); //deleteme
		}

		if (m->GetSpellMaxThreshold(i) != m->GetDefaultSpellMaxThreshold(i)) {
			auto e = BotSettingsRepository::BotSettings{
				.char_id = charID,
				.bot_id = botID,
				.setting_id = static_cast<uint16_t>(i),
				.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellMaxThreshold),
				.value = m->GetSpellMaxThreshold(i),
				.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellMaxThreshold),
				.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
			};

			v.emplace_back(e);

			LogBotSettings("{} says, 'Saving {} [{}] SpellMaxThreshold - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->GetDefaultSpellMaxThreshold(i)); //deleteme
		}

		if (m->IsBot()) {
			if (m->CastToBot()->GetSpellTypeAggroCheck(i) != m->CastToBot()->GetDefaultSpellTypeAggroCheck(i)) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeAggroCheck),
					.value = m->CastToBot()->GetSpellTypeAggroCheck(i),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeAggroCheck),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeAggroCheck - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypeAggroCheck(i)); //deleteme
			}

			if (m->CastToBot()->GetSpellTypeMinManaLimit(i) != m->CastToBot()->GetDefaultSpellTypeMinManaLimit(i)) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeMinManaPct),
					.value = m->CastToBot()->GetSpellTypeMinManaLimit(i),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeMinManaPct),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeMinManaPct - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypeMinManaLimit(i)); //deleteme
			}

			if (m->CastToBot()->GetSpellTypeMaxManaLimit(i) != m->CastToBot()->GetDefaultSpellTypeMaxManaLimit(i)) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeMaxManaPct),
					.value = m->CastToBot()->GetSpellTypeMaxManaLimit(i),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeMaxManaPct),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeMaxManaPct - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypeMaxManaLimit(i)); //deleteme
			}

			if (m->CastToBot()->GetSpellTypeMinHPLimit(i) != m->CastToBot()->GetDefaultSpellTypeMinHPLimit(i)) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeMinHPPct),
					.value = m->CastToBot()->GetSpellTypeMinHPLimit(i),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeMinHPPct),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeMinHPPct - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypeMinHPLimit(i)); //deleteme
			}

			if (m->CastToBot()->GetSpellTypeMaxHPLimit(i) != m->CastToBot()->GetDefaultSpellTypeMaxHPLimit(i)) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeMaxHPPct),
					.value = m->CastToBot()->GetSpellTypeMaxHPLimit(i),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeMaxHPPct),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeMaxHPPct - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypeMaxHPLimit(i)); //deleteme
			}

			if (m->CastToBot()->GetSpellTypePriority(i, BotPriorityCategories::Idle) != m->CastToBot()->GetDefaultSpellTypePriority(i, BotPriorityCategories::Idle, m->GetClass())) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeIdlePriority),
					.value = m->CastToBot()->GetSpellTypePriority(i, BotPriorityCategories::Idle),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeIdlePriority),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeIdlePriority - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypePriority(i, BotPriorityCategories::Idle, m->GetClass())); //deleteme
			}

			if (m->CastToBot()->GetSpellTypePriority(i, BotPriorityCategories::Engaged) != m->CastToBot()->GetDefaultSpellTypePriority(i, BotPriorityCategories::Engaged, m->GetClass())) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeEngagedPriority),
					.value = m->CastToBot()->GetSpellTypePriority(i, BotPriorityCategories::Engaged),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeEngagedPriority),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeEngagedPriority - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypePriority(i, BotPriorityCategories::Engaged, m->GetClass())); //deleteme
			}

			if (m->CastToBot()->GetSpellTypePriority(i, BotPriorityCategories::Pursue) != m->CastToBot()->GetDefaultSpellTypePriority(i, BotPriorityCategories::Pursue, m->GetClass())) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypePursuePriority),
					.value = m->CastToBot()->GetSpellTypePriority(i, BotPriorityCategories::Pursue),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypePursuePriority),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypePursuePriority - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypePriority(i, BotPriorityCategories::Pursue, m->GetClass())); //deleteme
			}

			if (m->CastToBot()->GetSpellTypeAEOrGroupTargetCount(i) != m->CastToBot()->GetDefaultSpellTypeAEOrGroupTargetCount(i)) {
				auto e = BotSettingsRepository::BotSettings{
					.char_id = charID,
					.bot_id = botID,
					.setting_id = static_cast<uint16_t>(i),
					.setting_type = static_cast<uint8_t>(BotSettingCategories::SpellTypeAEOrGroupTargetCount),
					.value = m->CastToBot()->GetSpellTypeAEOrGroupTargetCount(i),
					.category_name = m->CastToBot()->GetBotSpellCategoryName(BotSettingCategories::SpellTypeAEOrGroupTargetCount),
					.setting_name = m->CastToBot()->GetSpellTypeNameByID(i)
				};

				v.emplace_back(e);

				LogBotSettings("{} says, 'Saving {} [{}] SpellTypeAEOrGroupTargetCount - [{}] default [{}].'", m->GetCleanName(), m->GetSpellTypeNameByID(i), i, e.value, m->CastToBot()->GetDefaultSpellTypeAEOrGroupTargetCount(i)); //deleteme
			}
		}
	}
