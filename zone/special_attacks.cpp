...
int Mob::TryHeadShot(Mob *defender, EQ::skills::SkillType skillInUse)
{
	...
	if (
		...
		GetTarget() == defender &&
		(defender->IsActiveBodyType(BodyType::Humanoid) || !RuleB(Combat, HeadshotOnlyHumanoids)) &&
...
int Mob::TryAssassinate(Mob *defender, EQ::skills::SkillType skillInUse)
{
	if (
		...
		(defender->IsActiveBodyType(BodyType::Humanoid) || !RuleB(Combat, AssassinateOnlyHumanoids)) &&
