...
void bot_command_illusion_block(Client* c, const Seperator* sep)
{
	...
	if (helper_is_help_or_usage(sep->arg[1])) {
		...
		c->Message(Chat::White, "note: Set to 0 to allow bots to receive the illusion part of spells.");
		c->Message(Chat::White, "note: Set to 1 to block bots from receiving the illusion part of spells.");
	...
	for (auto my_bot : sbl) {
		...
		if (current_check) {
			c->Message(
				Chat::Green,
				fmt::format(
					"{} says, 'My current Illusion Block status is {}.'",
					my_bot->GetCleanName(),
					my_bot->GetIllusionBlock() ? "enabled" : "disabled"
				).c_str()
			);
		}
		else {
			my_bot->SetIllusionBlock(setStatus);
			++success_count;
		}
	}
	...
	if (!current_check) {
		if (success_count == 1 && first_found) {
			c->Message(
				Chat::Green,
				fmt::format(
					"{} says, 'My Illusion Block status was {}.'",
			...
			c->Message(
				Chat::Green,
				fmt::format(
					"{} of your bots set their Illusion Block status to {}.",
					success_count,
					setStatus ? "enabled" : "disabled"
				).c_str()
			);
