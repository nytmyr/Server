...
void bot_command_spell_holds(Client *c, const Seperator *sep)
{
	...
	if (sep->IsNumber(1)) {
		...
		if (spellType < Bot::BOT_SPELL_TYPE_START || spellType > Bot::BOT_SPELL_TYPE_END) {
			c->Message(Chat::Yellow, "You must choose a valid spell type. Spell types range from %i to %i", Bot::BOT_SPELL_TYPE_START, Bot::BOT_SPELL_TYPE_END);
