...
void bot_command_behind_mob(Client* c, const Seperator* sep)
{
	...
	if (helper_is_help_or_usage(sep->arg[1])) {
		c->Message(Chat::White, "usage: %s [current | value: 0-1] ([actionable: target | byname | ownergroup | ownerraid | targetgroup | namesgroup | healrotationtargets | mmr | byclass | byrace | spawned] ([actionable_name])).", sep->arg[0]);
	...
	for (auto my_bot : sbl) {
		...
		if (current_check) {
			c->Message(
				Chat::Green,
				fmt::format(
					"{} says, 'My current Behind Mob status is {}.'",
					my_bot->GetCleanName(),
