...
void bot_command_spell_pursue_priority(Client* c, const Seperator* sep)
{
	...
	if (helper_is_help_or_usage(sep->arg[1])) {
		...
		{
			"Sets the order of spell casts when the mob is",
			"fleeing in combat by spell type"
		};
		std::vector <std::string> notes = 
		{
			"-Setting a spell type to 0 will prevent that type",
			"from being cast.",
			"-If 2 or more are set to the same priority they",
			"will sort by spell type ID."
		};
		...
		{
			"To check the current pursue priority of buffs",
			"on all bots:",
			fmt::format(
				"{} {} current spawned",
				sep->arg[0],
