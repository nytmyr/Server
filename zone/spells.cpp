					...
					int16 resist_adjust, uint32 aa_id)
{
	...
	else if (cast_time && (IsClient() || IsBot()) && slot == CastingSlot::Item && item_slot != 0xFFFFFFFF) {
							...
							int32  mana_used, uint32 inventory_slot, int16 resist_adjust)
{
	...
	*/
	...
	if (IsBot() && DeleteChargeFromSlot >= 0) {
		...
		if (inst) {
			...
			if (!database.botdb.SaveItemBySlot(CastToBot(), DeleteChargeFromSlot, inst)) {
				...
				LogError("Failed to save item [{}] slot [{}] for [{}].", inst->GetID(), DeleteChargeFromSlot, GetCleanName());
				return;
			}
		}
		else {
...
int16 Mob::GetItemSlotToConsumeCharge(int32 spell_id, uint32 inventory_slot)
{
	...
	if ((!IsClient() && !IsBot()) || inventory_slot == 0xFFFFFFFF) {
